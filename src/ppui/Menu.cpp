...
void PPMenu::paint(PPGraphicsAbstract* g, pp_int32 px, pp_int32 py, pp_int32 menuSelection)
{
	...
	for (i = 0; i < items.size(); i++)
	{
		if (i == menuSelection)
		{
			...
			g->fillVerticalShaded(rect, nsbColor, nsdColor, true, *selectionColor);
			rect.y1 += h/2;
			rect.y2 += h/2 + 1;
			g->fillVerticalShaded(rect, nsbColor, nsdColor, false, *selectionColor);
